...
public class Shooter extends SubsystemBase {
  ...
  public Shooter() {
    ...
    shooterMotorLeft =
        KrakenBuilder.create(SHOOTER_MOTOR_LEFT_ID, CAN_BUS, "Shooter", "Shooter Motor Left")
            .withCurrentLimit(
    ...
    shooterMotorRight =
        KrakenBuilder.create(SHOOTER_MOTOR_RIGHT_ID, CAN_BUS, "Shooter", "Shooter Motor Right")
            .withCurrentLimit(
                new CurrentLimitsConfigs()
                    .withSupplyCurrentLimit(80)
                    .withSupplyCurrentLimitEnable(true))
    ...
    hoodedMotor =
        KrakenBuilder.create(HOODED_MOTOR_ID, CAN_BUS, "Shooter", "Hooder Motor")
            .withCurrentLimit(
                new CurrentLimitsConfigs()
  ...
  public Command setDesiredAngle() {
    if (ROBOT_POSE.getX() < 4.03 || ROBOT_POSE.getX() > FIELD_LENGTH_M - 4.03) {
      ...
      return setAngle(
          40); // maximize distance! along with the fact that the shooter can only go that far
    }
  }
    ...
    return run(
        () -> {
          ...
          shooterMotorRight.setControl(
              new VelocityVoltage(speed.getAsDouble()).withEnableFOC(true));
        });
  }
